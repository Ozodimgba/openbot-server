...
export class OpenBookV2Client {
  ...
  public async consumeEvents(
    ...
    let consumeEventsAdmin;
    if(market.consumeEventsAdmin.key){
      const eventAdminBs58 = market.consumeEventsAdmin.key.toBase58()
      consumeEventsAdmin = eventAdminBs58 == PublicKey.default.toBase58() ? null : market.consumeEventsAdmin.key;
    }
    ...
    const ix = await this.program.methods
      ...
      .accounts({
        eventHeap: market.eventHeap,
        market: marketPublicKey,
        consumeEventsAdmin: consumeEventsAdmin,
      })
      .remainingAccounts(accountsMeta)
      .instruction();
